module CoordinatorModule
  bloom do
    ...
    commit <~ (payloads * participants).pairs do |p, participant|
      [participant.client, p.payload, p.commit_bool] if p.num_votes == TwoPcProtocol::NUM_PARTICIPANTS && !sent_commit.include?([p.payload])
    end
    sent_commit <+ payloads { |p| [p.payload] if p.num_votes == TwoPcProtocol::NUM_PARTICIPANTS }
    ...
    coordinator_to_client <~ payloads do |p|
      [p.client, p.payload, p.commit_bool] if p.num_acks == TwoPcProtocol::NUM_PARTICIPANTS && !acked_client.include?([p.payload])
    end
    acked_client <+ payloads { |p| [p.payload] if p.num_acks == TwoPcProtocol::NUM_PARTICIPANTS }
  end
end
