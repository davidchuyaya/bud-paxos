  ...
  bloom do
    ...
    p1b <~ (p1a * max_acceptor_ballot).pairs do |incoming, ballot|
      if incoming.ballot_num >= ballot.num && incoming.id >= ballot.id
        [incoming.proposer_client, ip_port, incoming.partition, incoming.id, incoming.ballot_num, incoming.ballot_num, max_log.inspected]
      else
        [incoming.proposer_client, ip_port, incoming.partition, ballot.id, ballot.num, incoming.ballot_num, max_log.inspected]
      end
    end
    ...
    p2b <~ (p2a * max_acceptor_ballot).pairs do |incoming, ballot|
      if incoming.ballot_num >= ballot.num && incoming.id >= ballot.id
        [incoming.proposer_client, ip_port, incoming.partition, incoming.id, incoming.ballot_num, incoming.slot]
      else
        [incoming.proposer_client, ip_port, incoming.partition, ballot.id, ballot.num, incoming.slot]
      end
    end
